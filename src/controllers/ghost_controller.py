...
class GhostController:
    ...
    def _is_on_respawn_cooldown(self) -> bool:
        """Check if ghost is waiting for respawn cooldown to finish."""
    ...
    def _update_ai_direction(
        self,
        maze: Maze,
        target_position: Coordinate | None,
    ) -> None:
        """Query GhostAI for the next optimal direction according to state."""
        if self.ai is None:
            return
    ...
    def _check_home_arrival(self) -> None:
        """Transition returning ghost to home cooldown if home tile reached."""
        if (
    ...
    def prepare_next_step(
        ...
        self._update_ai_direction(maze, target_position)
